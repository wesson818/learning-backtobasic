...
const post = [{
    ...
    '1603784666647': 'Wen',
    '1604013599273': '12:56',
    '1604013595734': '2018-10-24T00:00:00.000Z',
...
query = {"$where":"this[1604013564441] != null && new Date(new Date(this[1604013564441]).setHours(0,0,0,0)).setFullYear(2000) == new Date(new Date().setHours(0,0,0,0) + " + 7 + "*86400000).setFullYear(2000)"};
// RANGE_NUM_OF_YEARS YEARS[2019,2021]
ltData = [1,3]
query = {"$where":'this[1604013595734] != null && ' + ltData[0] + ' <= (new Date() <= new Date(this[1604013595734]) ? 0 : Math.floor((new Date() - new Date(this[1604013595734]))/(1000*60*60*24*365))) && (new Date() <= new Date(this[1604013595734]) ? 0 : Math.floor((new Date() - this[1604013595734])/(1000*60*60*24*365))) <= ' + ltData[1]};
query = {"$where":ltData[0] + ' <= (new Date() <= new Date(this[1604013595734]) ? 0 : Math.floor((new Date() - new Date(this[1604013595734]))/(1000*60*60*24*365)))'};
